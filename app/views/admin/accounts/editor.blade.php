  ...
  <div class="panel panel-default">
    <div class="panel-heading" role="tab" id="addGLtab">
      <h4 class="panel-title">
        <a data-toggle="collapse" data-parent="#addGL" href="#addGLCollapse" aria-expanded="true" aria-controls="addGLCollapse">
...
{{Form::select('creditdebit', array('CREDIT' => 'Credit Account(+)', 'DEBIT' => 'Debit Account(-)'),null,array('class'=>'form-control'));}}  </div>
  <div class="form-group">
    ...
    @foreach($gl_data as $gl)
        ...
        <td>${{number_format($gl->NewAccountBalance,2)}}</td>
            @if($gl->EntryType == "ORDER")
                <td><a href='{{URL::to('/admin/orders/')}}/{{$gl->EntryTypeID}}' class='btn btn-default'>View</a></td>
            @else
                <td></td>   
            @endif
</tr>
...
<script>
    ...
    var data = [
        @foreach($gl_data as $gl)
        {entryid:"{{$gl->id}}",entrytype:"{{$gl->EntryType}}",credit:"${{number_format($gl->Credit,2)}}",debit:"${{number_format($gl->Debit,2)}}",newbal:"${{number_format($gl->NewAccountBalance,2)}}",
            @if($gl->EntryType == "ORDER")
